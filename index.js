...
function discountRedBox(oldprice, newprice) {
    if (newprice < oldprice) {
        return `<div class="box"><span style="color:white">-${(calculateDiscountRate(oldprice, newprice))}</span></div>`;
    }
    return ``;
}

function calculateDiscountRate(oldprice, newprice) {
    if (newprice < oldprice) {
        discountPrice = oldprice - newprice;
        divided = oldprice / 100;
...
function heartCount(count) {
    if (count > 0) {
        return `<i class="fas fa-heart">&nbsp;${count}</i>`;
    }
    return ``;
}

function detailArea(land, region, art, rebsorte) {
    if (!land == "") { land = '| ' + land; }
    if (!region == "") { region = '| ' + region; }
    if (!art == "") { art = '| ' + art; }
    if (!rebsorte == "") { rebsorte = '| ' + rebsorte; }
    return ` <p class="detail">${land} ${region}  ${art} ${rebsorte}</p>`;
...
function productsTemplate(product) {
    return `
        ...
        <div class="cards">
        <img src="${product.imageS}" alt="${product.productId}">
        <div class="card-img-overlay d-flex justify-content-start" style="padding: 0rem;padding-top: 20px;">
        <a href="#" class="card-link text-danger like">
        ${(discountRedBox(product.oldPrice, product.price))}
        ${(heartCount(product.params.likeCount))}
        </a>
      </div>
        <h6 class="name"><strong>${product.name}</strong></h6>
        ${(detailArea(product.params.land, product.params.region, product.params.art, product.params.rebsorte))}
        <p class="price"><span><strong>${product.priceText}*&nbsp;</strong></span> <span class="oldprice">${product.oldPriceText}</span></p>
        <p class="footer">${product.params.basePrice}</p>
         </div>
        </div>
      `;
}

document.getElementById("swiper-content").innerHTML = `
  
      ${products.map(productsTemplate).join("")}
      
    `;
document.getElementById("swiper-content2").innerHTML = `
  
      ${products.map(productsTemplate).join("")}
      
    `;

document.getElementById("swiper-content3").innerHTML = `
